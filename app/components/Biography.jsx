    ...
    records.map((item, key) => (
        <div className="flex justify-center items-center p-6" key={key}>
          <Link href={item.contributor} className="hover:no-underline">
            <img src={`${basePath}/${item.author_headshot}`} alt="" className="mx-auto rounded-full w-40 h-40 transition-all duration-300 cursor-pointer filter grayscale hover:grayscale-0"/>
